...
import type { ImportMapping } from '@decisionguru/shared';
...
importsRouter.post('/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    let parsed;
    if (ext === '.pdf') {
      parsed = await parsePdf(req.file.path, req.file.originalname);
    } else {
      parsed = parseWorkbook(req.file.path, req.file.originalname);
    }
...
importsRouter.post('/preview', (req, res) => {
  ...
  res.json({ rows, okCount, total: rows.length });
});
...
importsRouter.post('/commit', async (req, res) => {
  ...
  if (!mapping?.fileId) return res.status(400).json({ error: 'fileId required' });
  if (!getParsed(mapping.fileId)) return res.status(404).json({ error: 'File not found (re-upload)' });
  const rows = applyMapping(mapping).filter((r) => r.ok);
  ...
  for (const row of rows) {
    ...
    const dedupeKey = makeDedupeKey({ ...t, instrumentId });
  ...
  res.json({ imported, skipped, instruments: [...touched] });
});
